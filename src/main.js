...
class App {
    ...
    constructor() {
        ...
        this.recipeRenderer = new RecipeRenderer('recipe-grid', this.handleDeleteRecipe.bind(this), this.handleEditRecipe.bind(this), this.handleToggleFavorite.bind(this), this.handleViewRecipe.bind(this));
    ...
    handleViewRecipe(id) {
        ...
        if (imgEl)
            imgEl.src = recipe.imageUrl || AppConstants.DEFAULTS.IMAGE_URL;
        ...
        if (timeEl)
            timeEl.textContent = recipe.cookingTimeMinutes.toString();
    ...
    seedInitialData() {
        if (this.recipeService.getAllRecipes().length === 0) {
            ...
            if (recipes.length > 0 && recipes[0])
                this.recipeService.toggleFavorite(recipes[0].id);
            if (recipes.length > 4 && recipes[4])
                this.recipeService.toggleFavorite(recipes[4].id);
        }
    }
}
